...
def get_recent_runs(days: int = 7, ctx: Context = None) -> list[dict[str, Any]]:
    ...
    with tracer.start_as_current_span(
        "mcp.tool.get_recent_runs", context=parent
    ) as span:
...
def get_run_stats(
    ...
    parent = _parent_context_from(ctx) if ctx is not None else None
    with tracer.start_as_current_span(
        "mcp.tool.get_run_stats", context=parent
    ) as span:
        user_id = current_user_id()
        span.set_attribute("user_id", user_id)
...
@mcp.tool()
def get_weekly_stats(ctx: Context = None) -> dict[str, Any]:
    ...
    parent = _parent_context_from(ctx) if ctx is not None else None
    with tracer.start_as_current_span("mcp.tool.get_weekly_stats", context=parent):
...
@mcp.tool()
def calculate(expression: str, ctx: Context = None) -> str:
    ...
    parent = _parent_context_from(ctx) if ctx is not None else None
    with tracer.start_as_current_span("mcp.tool.calculate", context=parent):
        try:
