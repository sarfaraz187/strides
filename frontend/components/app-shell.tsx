      ...
      : section === "connectors"
        ...
        : section === "notifications"
          ...
          : section === "profile"
            ? "profile"
            : "dashboard";
  const activeConversationId = section === "chat" && segments[2] && segments[2] !== "list" ? segments[2] : undefined;

  return (
    <SidebarProvider className="h-screen w-full flex-col overflow-hidden bg-background lg:flex-row">
      <Sidebar active={active} locale={locale} activeConversationId={activeConversationId} className="hidden lg:flex" />
      {/*
