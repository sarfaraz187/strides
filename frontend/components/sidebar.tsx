...
import { Sidebar as SidebarPrimitive, SidebarContent, SidebarFooter, SidebarHeader, SidebarMenu, SidebarMenuButton, SidebarMenuItem, useSidebar } from "@/components/ui/sidebar";
...
export function Sidebar({ active, locale, className }: { active: "dashboard" | "coach" | "connectors" | "notifications" | "profile"; locale: string; className?: string }) {
  ...
  return (
    <SidebarPrimitive collapsible="none" className={cn("bg-sidebar py-7 transition-[width] duration-200 ease-linear", collapsed ? "w-19 px-3" : "w-60 px-5", className)}>
      <SidebarHeader className={cn("mb-9 flex-row items-center gap-2.5 p-0", collapsed && "justify-center")}>
